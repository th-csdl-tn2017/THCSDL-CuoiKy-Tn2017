...
namespace WeF_08_2013.Views
{
    public partial class VongDau : UserControl
    {
        ...
        {
            List<Models.VongDau> l = Controller.VongDau.get();
        ...
        {
            Models.VongDau x = new Models.VongDau();
            if (!check())
            {
            ...
            if (this.isEdit)
            {
                x.mahd = ((Models.VongDau)this.data.CurrentRow.DataBoundItem).mahd;
                if (!x.update())
                {
        ...
        private void province_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                Models.VongDau i = (item.DataBoundItem as Models.VongDau);
